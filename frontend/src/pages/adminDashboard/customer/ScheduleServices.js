...
// import './css/ScheduleServices.css';
...
const ScheduleServices = ({ isAdminView, isCreateMode }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const { formData, handleChange, handleSubmit, handleCancel, isEditMode } =
    useServiceForm({ location, navigate, isAdminView });
